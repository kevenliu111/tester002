...
if ($result->num_rows > 0) {
    ...
    $sql = "SELECT id,caption FROM inscontral_uploadcaption WHERE selected=1 ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
        ...
        $sql = "SELECT user.username,user.pwd FROM user LEFT JOIN uploaddata ON user.username=uploaddata.username AND uploaddata.photosrc='" . $photoid . "' WHERE user.creatstep>=10 AND user.errortimes<3 AND user.uploadset=1 AND uploaddata.id is null ";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            // 杈撳嚭鏁版嵁
            while ($row = $result->fetch_assoc()) {
            ...
            $photoFilename = "/www/wwwroot/insweb/inscontral/media/" . $photosrc;
         ...
         请聯繫上方ins賬號，本ins帳號僅作推廣帳號，不回覆任何問題🙅🙅圖片均為本店及客人實拍，如需分享請聯繫我們，未經許可請勿分享，感謝大家支持，感謝大家支持持";*/
            ...
            VALUES ('" . $username . "', '" . $photoid . "', '" . $captionid . "')";
        ...
        echo "None caption selected!";
    }
}else{
    echo "None img selected!";
}
